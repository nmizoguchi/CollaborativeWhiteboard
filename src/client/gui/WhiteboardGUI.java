...
public class WhiteboardGUI extends JFrame implements ClientListener {
    /**
     * This is the display portion that a user directly interacts with. It
     * contains a tool menu on the west side, a canvas in the center and a list
     * of online users and boards and a chat box on the east side. It also has a
     * File menu at the north. This display updates when the user makes an
     * action, by receiving messages from the server and calling different
     * runnable classes accordingly.
     * 
     * @author rcha
     */
     ...
     * 
     * @param message
     *            is a String that the user inputs in the chatbox
     */
    ...
    public void onNewuserMessageReceived(CWPMessage message) {
        ...
        SwingUtilities.invokeLater(new RunnableChat(this, user.getName()
    ...
    public void onDisconnecteduserMessageReceived(CWPMessage message) {
        ...
        SwingUtilities.invokeLater(new RunnableDisconnecteduser(activeUsers,
                user));
        SwingUtilities.invokeLater(new RunnableChat(this, user.getName()
    ...
    public void onChatMessageReceived(CWPMessage message) {
        ...
        SwingUtilities.invokeLater(new RunnableChat(this, chatMessage
                .toString()));
    }
    ...
    public void onReceiveUpdatedUsersOnBoard(CWPMessage message) {
        ...
        for(int i = 0; i < message.getArgumentsSize(); i+=2) {
            userList.add(new User(message.getArgument(i),message.getArgument(i+1)));
        }
        SwingUtilities.invokeLater(new RunnableUpdateUsers(activeUsers, userList));
    }
}
